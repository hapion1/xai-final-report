...
param_grid <- param_grid %>%
  ...
  update(tree_depth = tree_depth(range = c(1, 50))) %>%
  update(min_n = min_n(range = c(1, 20))) %>%
  update(loss_reduction = loss_reduction(range = c(0.01, 1)))

# Generate a regular grid  
xgb_param_grid <- grid_regular(param_grid, size = grid_size, levels = 2)
...
rf_param_grid <- rf_param_grid %>%
  ...
  #update(mtry = mtry(range = c(1, 10))) %>%
  update(min_n = min_n(range = c(1, 60)))

# Generate a regular grid
rf_param_grid <- grid_regular(rf_param_grid, size = grid_size,levels = 2)
...
svm_param_grid <- svm_param_grid %>%
  update(cost = cost(range = c(0.001, 100))) %>%
  update(rbf_sigma = rbf_sigma(range = c(0.00001, 0.01)))

# Generate a regular grid
svm_param_grid <- grid_regular(svm_param_grid, size = grid_size, levels = 2)
...
# Update parameter ranges for Logistic Regression
wlogreg_param_grid <- logreg_param_grid %>%
  update(penalty = penalty(range = c(0.001, 1))) %>%
  update(mixture = mixture(range = c(0.01, 1)))

# Generate a regular grid
logreg_param_grid <- grid_regular(logreg_param_grid, size = grid_size,levels = 2)
...
# Update parameter ranges for Decision Tree
dt_param_grid <- dt_param_grid %>%
  update(cost_complexity = cost_complexity(range = c(0.000001, 0.01))) %>%
  update(tree_depth = tree_depth(range = c(1, 20))) %>%
  update(min_n = min_n(range = c(1, 50)))

# Generate a regular grid
dt_param_grid <- grid_regular(dt_param_grid, size = grid_size, levels = 2)
...
cat("Total time taken:", total_duration, "seconds\n")
...
# Save the best model specifications to a text file
best_models_file <- "best_models.txt"
cat("Best Model Specifications:\n", file = best_models_file)

# List of models and their corresponding show_best() outputs
models_and_tunings <- list(
  xgb = xgb_tuning,
  rf = rf_tuning,
  svm = svm_tuning,
  logreg = logreg_tuning,
  dt = dt_tuning
)

for (model_name in names(models_and_tunings)) {
  cat("\n", model_name, ":\n", file = best_models_file, append = TRUE)
  cat("----------------------------\n", file = best_models_file, append = TRUE)
  
  tuning_output <- models_and_tunings[[model_name]] %>%
    show_best(metric = 'roc_auc', n = 1)
  
  # Access and format the tuning_output components here
  # Construct a formatted string based on the metrics
  
  cat("Formatted tuning_output string", file = best_models_file, append = TRUE)
}

# Print a message indicating that the best model specifications have been saved
cat("\nBest model specifications have been saved to", best_models_file, "\n")
